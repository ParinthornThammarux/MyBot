...
# + BACKTEST GRID STRATEGY จาก OHLCV /tradingview/history
# ============================================================
...
# Hysteresis: ต้องขยับห่างจาก last_trade_price >= MIN_MOVE_PCT_FROM_LAST_TRADE
...
# ------------------------------------------------------------
# [7] MAIN LOOP (GRID STRATEGY LIVE)
# ------------------------------------------------------------
...
# ============================================================
# [9] OHLCV /tradingview/history (ใช้สำหรับ BACKTEST)
# ============================================================

def _resolution_to_sec(res: str) -> int:
    """
    แปลง resolution ของ TradingView เป็นวินาทีต่อแท่ง
    '1','3','5','15','30','60','240','D' เป็นต้น
    """
    res = str(res).upper()
    if res.endswith("D"):
        days = int(res[:-1]) if res[:-1] else 1
        return days * 86400
    if res == "60":
        return 3600
    mapping = {
    ...
    return mapping.get(res, 300)  # default 5m


def fetch_candles_tv(sym: str, resolution: str = "5", lookback_bars: int = 2000) -> List[Dict[str, Any]]:
    """
    ดึงแท่งเทียนจาก /tradingview/history
    คืน List[{"ts","open","high","low","close","volume"}] เรียงจากเก่า -> ใหม่
    """
    now_sec = int(time.time())
    bar_sec = _resolution_to_sec(resolution)
    ...
    try:
        ...
        print(f"[BT ERROR] fetch_candles_tv http error: {e}")
        return []

    if not isinstance(data, dict) or data.get("s") != "ok":
        print(f"[BT ERROR] unexpected payload from tradingview/history: {data}")
        return []
...
def backtest_grid(
    ...
    initial_quote_thb: float = 100000.0
) -> Dict[str, Any]:
    """
    จำลอง GRID BOT บนแท่งเทียนย้อนหลัง
    - ใช้ logic เดียวกับ run_loop (grid_level, COOL_DOWN, HYSTERESIS ฯลฯ)
    - ไม่แตะ global position / grid_slots จริง
    - ใช้ wallet จำลอง: quote_avail, base_qty
    """
    print("=== BACKTEST GRID BOT ===")
    print(f"SYMBOL={sym}, RES={resolution}, BARS={lookback_bars}")
    print(f"GRID_CENTER={GRID_CENTER_PRICE}, STEP={GRID_STEP_PCT}% "
          f"DOWN={GRID_LEVELS_DOWN}, UP={GRID_LEVELS_UP}")
    print(f"ORDER_NOTIONAL={ORDER_NOTIONAL_THB} THB, FEE={FEE_RATE*100:.3f}%/side")
    est_profit = GRID_STEP_PCT - 2 * FEE_RATE * 100.0
    print(f"Est. profit per grid (before slippage) ≈ {est_profit:.3f}%")
    print(f"COOLDOWN_SEC={COOLDOWN_SEC}, MIN_MOVE_PCT_FROM_LAST_TRADE={MIN_MOVE_PCT_FROM_LAST_TRADE}%")
    print(f"Initial quote (THB) = {initial_quote_thb:.2f}")

    candles = fetch_candles_tv(sym, resolution=resolution, lookback_bars=lookback_bars)
    if not candles:
        print("[BT] no candles, abort.")
        return {}

    step_thb = grid_step_thb()

    # --- state ภายใน backtest ---
    quote_avail = initial_quote_thb  # THB wallet จำลอง
    base_qty = 0.0                   # ปริมาณ BASE ที่ถือ
    base_cost_thb = 0.0              # ต้นทุนรวม (รวม fee)
    realized_pnl = 0.0

    bt_grid_slots: Dict[int, float] = {}  # level -> qty
    last_bt_grid_level: Optional[int] = None
    last_bt_trade_price: Optional[float] = None
    last_bt_trade_ts: float = 0.0

    trades_log: List[Dict[str, Any]] = []  # เก็บทุก buy/sell

    def bt_avg_cost() -> float:
        return base_cost_thb / base_qty if base_qty > 0 else 0.0

    def bt_on_buy(level: int, px: float):
        nonlocal quote_avail, base_qty, base_cost_thb, last_bt_trade_price, last_bt_trade_ts
        # จำลองว่าตั้ง bid มูลค่า ORDER_NOTIONAL_THB
        if px <= 0:
            return False

        buy_value = ORDER_NOTIONAL_THB
        fee = buy_value * FEE_RATE
        total_cost = buy_value + fee

        if quote_avail < total_cost:
            # เงินไม่พอ
            return False

        qty = buy_value / px

        quote_avail -= total_cost
        base_qty += qty
        base_cost_thb += total_cost

        bt_grid_slots[level] = bt_grid_slots.get(level, 0.0) + qty

        last_bt_trade_price = px
        # last_bt_trade_ts ตั้งด้านนอกตาม ts ของแท่งนั้น

        trades_log.append({
            "type": "BUY",
            "ts": curr_ts,
            "dt": datetime.datetime.fromtimestamp(curr_ts),
            "price": px,
            "level": level,
            "qty": qty,
            "quote_after": quote_avail,
            "base_after": base_qty,
        })
        return True

    def bt_on_sell(sell_level: int, close_level: int, px: float):
        nonlocal quote_avail, base_qty, base_cost_thb, realized_pnl, last_bt_trade_price, last_bt_trade_ts
        if px <= 0:
            return False

        slot_qty = bt_grid_slots.get(close_level, 0.0)
        if slot_qty <= 0 or base_qty <= 0:
            return False

        target_qty = ORDER_NOTIONAL_THB / px
        qty_to_sell = min(target_qty, slot_qty, base_qty)
        if qty_to_sell <= 0:
            return False

        # อัพเดต grid slot
        new_slot_qty = slot_qty - qty_to_sell
        if new_slot_qty <= 1e-12:
            bt_grid_slots.pop(close_level, None)
        else:
            bt_grid_slots[close_level] = new_slot_qty
        ...
        # ต้นทุนส่วนของ qty นี้
        portion = qty_to_sell / base_qty
        cost_part = base_cost_thb * portion
        pnl = proceed - cost_part

        realized_pnl += pnl
        base_qty -= qty_to_sell
        base_cost_thb -= cost_part
    ...
    for i, c in enumerate(candles):
        ...
        raw_idx = grid_level_from_price(px)
        lvl = math.floor(raw_idx)

        buy_level_cell = lvl
        sell_level_cell = lvl + 1

        # ถ้าอยู่นอกกรอบ grid ให้ถือเฉย ๆ
        if buy_level_cell < -GRID_LEVELS_DOWN or sell_level_cell > GRID_LEVELS_UP:
            last_bt_grid_level = lvl
            continue

        # init รอบแรก
        if last_bt_grid_level is None:
            last_bt_grid_level = lvl
            continue

        moved_down = lvl < last_bt_grid_level
        moved_up = lvl > last_bt_grid_level
        ...
        # cooldown
        if last_bt_trade_ts > 0 and (curr_ts - last_bt_trade_ts) < COOLDOWN_SEC:
            last_bt_grid_level = lvl
            continue

        # hysteresis
        if last_bt_trade_price is not None and last_bt_trade_price > 0:
            move_pct = abs(px - last_bt_trade_price) / last_bt_trade_price * 100.0
            if move_pct < MIN_MOVE_PCT_FROM_LAST_TRADE:
                last_bt_grid_level = lvl
                continue
        ...
        # ---- BUY: เลเวลลดลง ----
        if moved_down:
            for grid_lvl in range(last_bt_grid_level - 1, lvl - 1, -1):
                if grid_lvl < -GRID_LEVELS_DOWN:
                    break

                # เงินไม่พอเปิด grid เพิ่ม
                buy_px = round(grid_price(grid_lvl), PRICE_ROUND)
                if not bt_on_buy(grid_lvl, buy_px):
                    break
                traded = True

        # ---- SELL: เลเวลเพิ่มขึ้น ----
        elif moved_up:
            if base_qty <= 0 or not bt_grid_slots:
                # ไม่มีของขาย
                pass
            else:
                for grid_lvl in range(last_bt_grid_level + 1, lvl + 1):
                    sell_level = grid_lvl
                    if sell_level > GRID_LEVELS_UP:
                        break
                    close_level = sell_level - 1
                    sell_px = round(grid_price(sell_level), PRICE_ROUND)
                    if bt_on_sell(sell_level, close_level, sell_px):
                        traded = True

        if traded:
            last_bt_trade_ts = curr_ts

        last_bt_grid_level = lvl

    # หลังจบ ถ้ายังถือ BASE อยู่ ให้ mark-to-market มูลค่า
    last_px = candles[-1]["close"]
    equity_final = quote_avail + base_qty * last_px
    equity_initial = initial_quote_thb
    total_pnl = equity_final - equity_initial
    roi_pct = (total_pnl / equity_initial) * 100.0

    print("===== BACKTEST GRID RESULT =====")
    print(f"Initial quote  : {equity_initial:.2f} THB")
    print(f"Final equity   : {equity_final:.2f} THB")
    print(f"Total PnL      : {total_pnl:.2f} THB ({roi_pct:.2f}%)")
    print(f"Realized PnL   : {realized_pnl:.2f} THB")
    print(f"Final BASE qty : {base_qty:.6f} {BASE_ASSET} @ last_px={last_px:.4f}")
    print(f"Open grid slots: {len(bt_grid_slots)}")

    if trades_log:
        print(f"Num trades     : {len(trades_log)}")
        print("--- Last 5 trades ---")
    ...
    return {
        ...
        "final_base_qty": base_qty,
        "last_price": last_px,
        "trades": trades_log,
        "grid_slots": bt_grid_slots,
    }
...
if __name__ == "__main__":
    # 1) โหมด BACKTEST
    #    ปรับ resolution / lookback_bars / initial_quote ตามใจ
    result = backtest_grid(
        sym=SYMBOL,
        resolution="5",        # เช่น 5 นาที, หรือ "15","60","240","1D" ฯลฯ
        lookback_bars=2000,    # จำนวนแท่งย้อนหลัง
        initial_quote_thb=100000.0
    )

    # 2) ถ้าจะรันบอทจริง (loop ไม่จบ) ให้คอมเมนต์ backtest ด้านบน แล้วใช้:
    # run_loop()
